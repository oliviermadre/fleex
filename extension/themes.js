'use strict';
// Built-in Fleex theme palettes, generated verbatim from
// packages/web/src/lib/themes.ts (BUILT_IN_THEMES). Custom themes are read at
// runtime from each workspace's app_config, so only the built-ins are embedded.
...
// Rough perceived-luminance check so custom themes (no `light` flag) still pick
// a sensible light/dark mode from their background.
...
// Resolve { colors, light } from an activeThemeId + the workspace's customThemes.
...
// Apply a resolved theme by writing CSS custom properties on :root.
function applyTheme(resolved) {
  const root = document.documentElement;
  if (!resolved) { root.removeAttribute('data-theme'); root.removeAttribute('style'); return; }
