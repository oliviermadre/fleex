...
/**
 * Spawn a service in detached mode with stdio fully redirected to a log file.
 * The detached flag creates a new process group so `kill(-pid)` on stop kills
 * every descendant in one go.
 */
...
/**
 * Run a synchronous build/install command, capturing output to a log file.
 * Resolves on success, rejects on non-zero exit so callers can fall back.
 */
function runCommand(
  cmd: string,
  args: string[],
