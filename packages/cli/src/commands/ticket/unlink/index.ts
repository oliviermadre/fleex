...
const def: CommandDef = {
  ...
  name: 'unlink',
  description:
    'Unlink repositories / PRs / issues from a ticket — --pr and --issue accept a full GitHub URL or org/name#N (unlink <id> --repo org/name | --pr <pr-url|org/name#n> | --issue <issue-url|org/name#n>)',
  setup(cmd) {
    cmd.argument('<id>', 'Ticket display ID or UUID');
    cmd.option(
      '--repo <org/name>',
      'Repository to unlink (repeatable)',
      accumulate,
      [] as string[],
    );
    cmd.option(
      '--pr <url|org/name#n>',
      'GitHub PR to unlink — full PR URL or org/name#N (repeatable)',
      accumulate,
      [] as string[],
    );
    cmd.option(
      '--issue <url|org/name#n>',
      'GitHub issue to unlink — full issue URL or org/name#N (repeatable)',
      accumulate,
      [] as string[],
    );
