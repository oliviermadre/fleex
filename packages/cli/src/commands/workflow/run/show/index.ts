  ...
  action: async (runId: string, opts: ShowOptions) => {
    ...
    present(detail, () => {
      ...
      const stepName = new Map(
        (run.templateSnapshot?.steps ?? []).map((s) => [s.id, s.name ?? s.id]),
      );
      ...
      process.stdout.write(
        `  ${c.bold('Current step:')} ${run.currentStepId ? (stepName.get(run.currentStepId) ?? run.currentStepId) : '-'}\n`,
      );
      process.stdout.write(
        `  ${c.bold('Triggered by:')} ${run.triggeredBy ?? '-'} (${run.triggeredFrom ?? '-'})\n`,
      );
      ...
      if (stepRuns.length === 0) {
        ...
        for (const s of stepRuns) {
          ...
          process.stdout.write(
            `    ${c.dim(s.id)}  ${s.status.padEnd(12)} attempt ${s.attempt}  ${name}${res}\n`,
          );
        }
      }
      process.stdout.write('\n');
