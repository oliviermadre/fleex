...
async function openDb(ws: Workspace): Promise<Db | null> {
  ...
  if (driver === 'supabase') {
    ...
    if (!url) {
      console.error(`  ⚠ ${ws.name}: no FLEEX_SUPABASE_DB_URL — skipping`);
      return null;
    }
  ...
  if (driver === 'sqlite') {
    ...
    if (!p || !existsSync(p)) {
      console.error(`  ⚠ ${ws.name}: sqlite path missing — skipping`);
      return null;
    }
    return openSqlite(p);
  }
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (c.unknown.size > 0) {
        unknownModel++;
        console.error(
          `  ⚠ unknown model(s) ${[...c.unknown].join(',')} in ${sid} — skipped (no partial write)`,
        );
        continue;
      }
      costSum += c.cost;
...
async function main() {
  ...
  if (selected.length === 0) {
    console.error(
      `No workspace matched "${WS_ARG}". Available: ${all.map((w) => w.name).join(', ')}`,
    );
    process.exit(1);
  }
