...
export interface WakeScope {
  /**
   * Where the new content landed. `null` / absent = the main stream: only agents
   * waiting in the main stream wake, agents parked inside an assistant thread
   * stay parked (the assistant alone drives them). A thread id = a turn posted
   * inside that thread: only that thread's waiting agent wakes.
   */
...
export class WakeWaitingAgentsUseCase {
  constructor(
    ...
    private readonly commentStore?: Pick<CommentStorePort, 'getById'>,
  ) {}

  /**
   * Wake up agents that are waiting_for_info on a ticket.
   ...
   * @param scope - Main stream vs. a given assistant thread (see WakeScope).
   *   A waiting mention belongs to a thread when the comment that created it
   *   carries a `threadId`.
   *
   ...
   */
  async execute(ticketId: string, excludeAgentNames?: readonly string[], scope: WakeScope = {}): Promise<void> {
    const excluded = new Set(excludeAgentNames ?? []);
    const waitingMentions = await this.mentionStore.getWaitingByTicket(ticketId);
    const wantedThread = scope.threadId ?? null;
    ...
    for (const mention of waitingMentions) {
      ...
      if (this.commentStore) {
        const mentionThread = (await this.commentStore.getById(mention.commentId))?.threadId ?? null;
        if (mentionThread !== wantedThread) continue;
      }
