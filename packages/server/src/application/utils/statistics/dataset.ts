...
import type {
  ...
  PanelEventRow, WorkflowRunRow, TicketMove, NamedRef, RoutineRef,
} from './rows.js';
...
export interface StatsDataset {
  ...
  readonly routineById: ReadonlyMap<string, RoutineRef>;
...
export function buildDataset(raw: RawStatsData): StatsDataset {
  ...
  const mentions: MentionRow[] = raw.mentions.map((entity) => {
    ...
    return { id: m.id, ticketId: m.ticketId, status: m.status, createdAtMs: Date.parse(m.createdAt) };
  });
  ...
  return {
    ...
    routineById: new Map(raw.routines.map((r) => [r.id, r])),
    movesByTicket,
  };
}
