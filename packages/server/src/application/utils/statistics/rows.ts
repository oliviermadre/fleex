...
export interface WorkflowRunRow {
  /** Null iff the run belongs to a routine; such runs have no per-ticket count. */
  readonly ticketId: string | null;
  /**
   * Null for a synthetic run — a routine targeting a primitive has no template.
   * Those runs stay off the workflow board, where they would all collapse into
   * one meaningless row.
   */
  readonly templateId: string | null;
  readonly templateName: string | null;
  /** Set iff the run was launched by a routine. */
...
/**
 * Minimal shape of a routine needed by its leaderboard.
 *
 * Kept separate from {@link NamedRef} because the routine board also shows what
 * the routine launches, which no other board has.
 */
export interface RoutineRef {
  readonly id: string;
  readonly name: string;
  readonly target: { readonly kind: string; readonly ref: string };
}
