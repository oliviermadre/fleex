...
export class JsonWorkflowRunStore implements WorkflowRunStorePort {
  ...
  private async loadFromDisk(): Promise<void> {
    ...
    try {
      ...
      for (const r of data) {
        this.runs.set(r.id, new WorkflowRunEntity(
          r.id, r.ticketId, r.templateId, r.templateSnapshot, r.status,
          r.currentStepId ?? null, r.triggeredBy, r.triggeredFrom,
          new Date(r.startedAt), r.completedAt ? new Date(r.completedAt) : null,
  ...
  private async doWrite(): Promise<void> {
    try {
      const data: SerializedRun[] = Array.from(this.runs.values()).map((r) => ({
        id: r.id, ticketId: r.ticketId, templateId: r.templateId,
        templateSnapshot: r.templateSnapshot, status: r.status,
        currentStepId: r.currentStepId, triggeredBy: r.triggeredBy,
