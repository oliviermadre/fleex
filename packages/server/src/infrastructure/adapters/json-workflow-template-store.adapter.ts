...
export class JsonWorkflowTemplateStore implements WorkflowTemplateStorePort {
  ...
  private async loadFromDisk(): Promise<void> {
    ...
    try {
      ...
      for (const t of data) {
        this.templates.set(t.id, new WorkflowTemplateEntity(
          t.id, t.name, t.slug, t.emoji ?? '', t.description ?? '',
          t.steps ?? [], t.edges ?? [], t.entryStepId, t.enabled,
  ...
  private async syncToDisk(): Promise<void> {
    try {
      const data: SerializedTemplate[] = Array.from(this.templates.values()).map((t) => ({
        ...
        steps: t.steps, edges: t.edges, entryStepId: t.entryStepId, enabled: t.enabled,
