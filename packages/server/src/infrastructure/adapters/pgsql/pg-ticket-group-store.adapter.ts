...
function rowToEntity(row: any, boardIds: string[]): TicketGroupEntity {
  return new TicketGroupEntity(
    ...
    Boolean(row.blocked), Boolean(row.favorite),
...
export class PgTicketGroupStore implements TicketGroupStorePort {
  ...
  async getAllTicketGroups(): Promise<TicketGroupEntity[]> {
    ...
    return rows.map((r: any) => rowToEntity(r, boardMap.get(r.id) ?? (r.board_id ? [r.board_id] : [])));
  }
  ...
  async getTicketGroupById(id: string): Promise<TicketGroupEntity | null> {
    ...
    const { rows: assocRows } = await this.db.query('SELECT board_id FROM ticket_group_boards WHERE group_id = $1', [id]);
    const boardIds = assocRows.map((a: any) => a.board_id as string);
    return rowToEntity(rows[0], boardIds.length > 0 ? boardIds : (rows[0].board_id ? [rows[0].board_id] : []));
  }

  async getTicketGroupsByBoard(boardId: string): Promise<TicketGroupEntity[]> {
    const { rows } = await this.db.query(
      'SELECT tg.* FROM ticket_groups tg INNER JOIN ticket_group_boards tgb ON tg.id = tgb.group_id WHERE tgb.board_id = $1', [boardId],
    );
  ...
  async saveTicketGroup(group: TicketGroupEntity): Promise<void> {
    await this.db.query(
      ...
      [group.id, group.boardIds[0] ?? null, group.name, group.emoji, group.color,
    ...
    for (const bid of group.boardIds) {
      await this.db.query('INSERT INTO ticket_group_boards (group_id, board_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [group.id, bid]);
    }
  }
  ...
  async getBoardIdsByGroup(groupId: string): Promise<string[]> {
    const { rows } = await this.db.query('SELECT board_id FROM ticket_group_boards WHERE group_id = $1', [groupId]);
    return rows.map((r: any) => r.board_id as string);
  }

  async addBoardToGroup(groupId: string, boardId: string): Promise<void> {
    await this.db.query('INSERT INTO ticket_group_boards (group_id, board_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [groupId, boardId]);
  }

  async removeBoardFromGroup(groupId: string, boardId: string): Promise<void> {
    await this.db.query('DELETE FROM ticket_group_boards WHERE group_id = $1 AND board_id = $2', [groupId, boardId]);
  }
  ...
  async addMembership(ticketId: string, groupId: string): Promise<void> {
    await this.db.query('INSERT INTO ticket_group_memberships (ticket_id, group_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [ticketId, groupId]);
  }

  async removeMembership(ticketId: string, groupId: string): Promise<void> {
    await this.db.query('DELETE FROM ticket_group_memberships WHERE ticket_id = $1 AND group_id = $2', [ticketId, groupId]);
  }
  ...
  async addRelationship(parentId: string, childId: string): Promise<void> {
    await this.db.query('INSERT INTO ticket_relationships (parent_id, child_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [parentId, childId]);
  }

  async removeRelationship(parentId: string, childId: string): Promise<void> {
    await this.db.query('DELETE FROM ticket_relationships WHERE parent_id = $1 AND child_id = $2', [parentId, childId]);
  }

  async removeRelationshipsByTicket(ticketId: string): Promise<void> {
    await this.db.query('DELETE FROM ticket_relationships WHERE parent_id = $1 OR child_id = $1', [ticketId]);
  }
}
