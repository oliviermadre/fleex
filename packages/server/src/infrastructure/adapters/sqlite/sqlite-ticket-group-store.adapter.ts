...
export class SqliteTicketGroupStoreAdapter implements TicketGroupStorePort {
  ...
  async getAllTicketGroups(): Promise<TicketGroupEntity[]> {
    ...
    const allAssoc = this.conn.db
      .prepare('SELECT * FROM ticket_group_boards')
      .all() as BoardAssocRow[];
    ...
    return rows.map((r) =>
      this.toEntity(r, boardMap.get(r.id) ?? (r.board_id ? [r.board_id] : [])),
    );
  }

  async getTicketGroupById(id: string): Promise<TicketGroupEntity | null> {
    const row = this.conn.db.prepare('SELECT * FROM ticket_groups WHERE id = ?').get(id) as
      TicketGroupRow | undefined;
    if (!row) return null;
    const boardIds = (
      this.conn.db
        .prepare('SELECT board_id FROM ticket_group_boards WHERE group_id = ?')
        .all(id) as BoardAssocRow[]
    ).map((a) => a.board_id);
  ...
  async saveTicketGroup(group: TicketGroupEntity): Promise<void> {
    ...
    const insertAssoc = this.conn.db.prepare(
      'INSERT OR IGNORE INTO ticket_group_boards (group_id, board_id) VALUES (?, ?)',
    );
  ...
  async getBoardIdsByGroup(groupId: string): Promise<string[]> {
    return (
      this.conn.db
        .prepare('SELECT board_id FROM ticket_group_boards WHERE group_id = ?')
        .all(groupId) as BoardAssocRow[]
    ).map((a) => a.board_id);
  }

  async addBoardToGroup(groupId: string, boardId: string): Promise<void> {
    this.conn.db
      .prepare('INSERT OR IGNORE INTO ticket_group_boards (group_id, board_id) VALUES (?, ?)')
      .run(groupId, boardId);
  }

  async removeBoardFromGroup(groupId: string, boardId: string): Promise<void> {
    this.conn.db
      .prepare('DELETE FROM ticket_group_boards WHERE group_id = ? AND board_id = ?')
      .run(groupId, boardId);
  }
  ...
  async getMembershipsByGroup(groupId: string): Promise<TicketGroupMembership[]> {
    return (
      this.conn.db
        .prepare('SELECT * FROM ticket_group_memberships WHERE group_id = ?')
        .all(groupId) as MembershipRow[]
    ).map((r) => ({ ticketId: r.ticket_id, groupId: r.group_id }));
  }

  async getMembershipsByTicket(ticketId: string): Promise<TicketGroupMembership[]> {
    return (
      this.conn.db
        .prepare('SELECT * FROM ticket_group_memberships WHERE ticket_id = ?')
        .all(ticketId) as MembershipRow[]
    ).map((r) => ({ ticketId: r.ticket_id, groupId: r.group_id }));
  }

  async addMembership(ticketId: string, groupId: string): Promise<void> {
    this.conn.db
      .prepare('INSERT OR IGNORE INTO ticket_group_memberships (ticket_id, group_id) VALUES (?, ?)')
      .run(ticketId, groupId);
  }

  async removeMembership(ticketId: string, groupId: string): Promise<void> {
    this.conn.db
      .prepare('DELETE FROM ticket_group_memberships WHERE ticket_id = ? AND group_id = ?')
      .run(ticketId, groupId);
  }
  ...
  async getChildRelationships(parentId: string): Promise<TicketRelationship[]> {
    return (
      this.conn.db
        .prepare('SELECT * FROM ticket_relationships WHERE parent_id = ?')
        .all(parentId) as RelationshipRow[]
    ).map((r) => ({ parentId: r.parent_id, childId: r.child_id }));
  }

  async getParentRelationships(childId: string): Promise<TicketRelationship[]> {
    return (
      this.conn.db
        .prepare('SELECT * FROM ticket_relationships WHERE child_id = ?')
        .all(childId) as RelationshipRow[]
    ).map((r) => ({ parentId: r.parent_id, childId: r.child_id }));
  }

  async addRelationship(parentId: string, childId: string): Promise<void> {
    this.conn.db
      .prepare('INSERT OR IGNORE INTO ticket_relationships (parent_id, child_id) VALUES (?, ?)')
      .run(parentId, childId);
  }

  async removeRelationship(parentId: string, childId: string): Promise<void> {
    this.conn.db
      .prepare('DELETE FROM ticket_relationships WHERE parent_id = ? AND child_id = ?')
      .run(parentId, childId);
  }

  async removeRelationshipsByTicket(ticketId: string): Promise<void> {
    this.conn.db
      .prepare('DELETE FROM ticket_relationships WHERE parent_id = ? OR child_id = ?')
