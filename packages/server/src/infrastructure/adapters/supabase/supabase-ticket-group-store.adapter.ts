...
export class SupabaseTicketGroupStore implements TicketGroupStorePort {
  ...
  async getAllTicketGroups(): Promise<TicketGroupEntity[]> {
    ...
    return (data as TicketGroupRow[]).map((r) =>
  ...
  async getTicketGroupById(id: string): Promise<TicketGroupEntity | null> {
    ...
    if (!data) return null;
    const { data: assocData } = await this.conn.client
      .from('ticket_group_boards')
      .select('board_id')
      .eq('group_id', id);
    const boardIds = ((assocData ?? []) as BoardAssocRow[]).map((a) => a.board_id);
    return rowToEntity(
      data as TicketGroupRow,
      boardIds.length > 0 ? boardIds : data.board_id ? [data.board_id] : [],
    );
  }

  async getTicketGroupsByBoard(boardId: string): Promise<TicketGroupEntity[]> {
    // Get group IDs from junction, then fetch groups
    const { data: assocData, error: assocErr } = await this.conn.client
      .from('ticket_group_boards')
      .select('group_id')
      .eq('board_id', boardId);
    ...
    const { data: allAssocData } = await this.conn.client
      .from('ticket_group_boards')
      .select('*')
  ...
  async saveTicketGroup(group: TicketGroupEntity): Promise<void> {
    ...
    if (group.boardIds.length > 0) {
      const { error: assocErr } = await this.conn.client
        .from('ticket_group_boards')
        .upsert(group.boardIds.map((bid) => ({ group_id: group.id, board_id: bid })));
  ...
  async addBoardToGroup(groupId: string, boardId: string): Promise<void> {
    const { error } = await this.conn.client
      .from('ticket_group_boards')
      .upsert({ group_id: groupId, board_id: boardId }, { onConflict: 'group_id,board_id' });
  ...
  async addMembership(ticketId: string, groupId: string): Promise<void> {
    const { error } = await this.conn.client
      .from('ticket_group_memberships')
      .upsert({ ticket_id: ticketId, group_id: groupId }, { onConflict: 'ticket_id,group_id' });
  ...
  async removeRelationshipsByTicket(ticketId: string): Promise<void> {
    const { error: e1 } = await this.conn.client
      .from('ticket_relationships')
      .delete()
      .eq('parent_id', ticketId);
    if (e1) throw new Error(`Failed to remove relationships: ${e1.message}`);
    const { error: e2 } = await this.conn.client
      .from('ticket_relationships')
      .delete()
      .eq('child_id', ticketId);
