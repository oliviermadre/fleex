...
export class SupabaseTicketStore implements TicketStorePort {
  ...
  async createTicket(ticket: TicketEntity): Promise<void> {
    ...
    // We omit display_id from the insert payload so the DB default fires,
    // then read back the assigned value.
    const { data, error } = await this.conn.client
      .from('tickets')
      .insert({
        id: ticket.id,
      ...
      .select('display_id')
      .single();
    if (error) throw new Error(`SupabaseTicketStore.createTicket failed: ${error.message}`);
    if (!data) throw new Error('createTicket failed to return display_id');
    ticket.displayId = (data as { display_id: number }).display_id;
  }
