 ...
 * says *why*.
 */
export function registerWorkflowGuard(app: FastifyInstance, capabilities: ServerCapabilities): void {
  app.addHook('preHandler', async (_request, reply) => {
    ...
    return reply.code(503).send({
      ...
      message: `Workflows are not available on the "${capabilities.storageDriver}" storage driver. `
