...
const EDGES: WorkflowEdge[] = [{ id: 'edge-1', source: 'step-1', target: 'step-2' }];

function makeTemplate(
  overrides: Partial<{ id: string; slug: string; name: string; enabled: boolean }> = {},
) {
  ...
  describe(`workflow store contract — ${driverName}`, () => {
    ...
    describe('WorkflowTemplateStorePort', () => {
      ...
      it('excludes disabled templates from getEnabled but keeps them in getAll', async () => {
        await h.templateStore.save(
          makeTemplate({ id: 'tmpl-1', slug: 'enabled-one', name: 'Alpha' }),
        );
        await h.templateStore.save(
          makeTemplate({ id: 'tmpl-2', slug: 'disabled-one', name: 'Beta', enabled: false }),
        );
    ...
    describe('StepRunStorePort', () => {
      ...
      it('lists step runs for a workflow run', async () => {
        ...
        await h.stepRunStore.save(
          StepRunEntity.create({ id: 'sr-2', workflowRunId: 'run-1', stepId: 'step-2' }),
        );
