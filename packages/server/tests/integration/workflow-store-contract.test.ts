...
if (!PGSQL_URL) {
  ...
  runWorkflowStoreContract('pgsql', async (): Promise<WorkflowStoreHarness> => {
    ...
    const { PgWorkflowTemplateStore } =
      await import('../../src/infrastructure/adapters/pgsql/pg-workflow-template-store.adapter.js');
    const { PgWorkflowRunStore } =
      await import('../../src/infrastructure/adapters/pgsql/pg-workflow-run-store.adapter.js');
    const { PgStepRunStore } =
      await import('../../src/infrastructure/adapters/pgsql/pg-step-run-store.adapter.js');
    const { PgTicketStore } =
      await import('../../src/infrastructure/adapters/pgsql/pg-ticket-store.adapter.js');
