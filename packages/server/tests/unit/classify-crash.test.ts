...
describe('classifyCrash', () => {
  ...
  it('classifies an org usage-limit crash', () => {
    const c = classifyCrash("Claude Code process exited with error result: You've hit your org's monthly usage limit.", { acknowledged: true });
  ...
  it('classifies a max-turns crash', () => {
    ...
    expect(classifyCrash('reached the maximum turns', { acknowledged: true }).reason).toBe('max_turns');
  });
  ...
  it('falls back to startup_error before acknowledge, preserving the raw message', () => {
    const c = classifyCrash('Could not create workspace directory for ticket', { acknowledged: false });
