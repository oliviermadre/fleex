...
class LatentHostFs implements HostFs {
  ...
  async readdir(): Promise<{ name: string; isFile: boolean; isDirectory: boolean }[]> { return []; }
  async stat(): Promise<{ size: number; mtimeMs: number } | null> { return null; }
  async rm(): Promise<void> {}
  async readTail(): Promise<string> { return ''; }
}
...
describe('JSON workflow stores — concurrent writes', () => {
  it('persists all 20 step runs saved concurrently', async () => {
    ...
    await Promise.all(
      Array.from({ length: 20 }, (_, i) =>
        store.save(StepRunEntity.create({ id: `sr-${i}`, workflowRunId: 'run-1', stepId: 'step-1', attempt: i + 1 }))),
    );
  ...
  it('persists all workflow runs saved concurrently', async () => {
    ...
    await Promise.all(
      Array.from({ length: 20 }, (_, i) =>
        store.save(WorkflowRunEntity.create({
          id: `run-${i}`, ticketId: 't-1', templateId: 'tmpl-1',
          templateSnapshot: SNAPSHOT, triggeredBy: '@t', triggeredFrom: 'test',
        }))),
    );
  ...
  it('reloads persisted step runs from disk after a restart', async () => {
    ...
    await first.init();
    await first.save(StepRunEntity.create({ id: 'sr-1', workflowRunId: 'run-1', stepId: 'step-1' }));
