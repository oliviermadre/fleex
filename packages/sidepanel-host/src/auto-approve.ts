 ...
 */
import { isAutoApproveActive, type AutoApprove, type SessionStore } from './sessions.ts';
...
export function applyConfirm(
  ...
  store: SessionStore,
): { pending: PendingConfirm; approved: boolean; allowlistChanged: boolean } | null {
  ...
  if (approved && scope) {
    if (scope === 'session') store.setAutoApprove(entry.sessionId, { all: true, tools: [] });
    else store.allowTool(entry.sessionId, entry.name);
    allowlistChanged = true;
  }
  return { pending: entry, approved, allowlistChanged };
}
 ...
 * against, so a standing approval granted before it must not survive it.
 * Returns true when something was actually disarmed (nothing to tell the user
 * otherwise).
 */
export function disarmForPage(sessionId: string, store: SessionStore): boolean {
  const s = store.get(sessionId);
  if (!s || !isAutoApproveActive(s.autoApprove)) return false;
