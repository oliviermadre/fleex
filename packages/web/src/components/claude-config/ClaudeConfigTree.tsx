...
export function ClaudeConfigTree() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Tree content */}
      <div className="flex-1 overflow-y-auto py-1" onContextMenu={handleTreeContextMenu}>
        {treeLoading && tree.length === 0 ? (
          <div className="px-4 py-3 text-xs text-[var(--theme-text-muted)]">Loading...</div>
        ) : tree.length === 0 ? (
          <div className="px-4 py-3 text-xs text-[var(--theme-text-muted)]">
            No config files found
          </div>
        ) : (
          tree.map((entry) => <TreeNode key={entry.relativePath} entry={entry} depth={0} />)
        )}
      </div>
  ...
  if (entry.isDirectory) {
    ...
    return (
      <>
        ...
        {isExpanded &&
          entry.children?.map((child: ClaudeConfigTreeEntry) => (
            <TreeNode key={child.relativePath} entry={child} depth={depth + 1} />
          ))}
        {isExpanded && showInlineInput && (
          <InlineCreateInput depth={depth + 1} type={creatingEntry!.type} />
        )}
      </>
    );
  }
  ...
  if (ext === 'json') {
    return (
      <svg
        ...
        strokeLinecap="round"
        strokeLinejoin="round"
        className={cn('flex-shrink-0', tintText('yellow'))}
      >
  ...
  if (ext === 'md') {
    return (
      <svg
        ...
        strokeLinecap="round"
        strokeLinejoin="round"
        className={cn('flex-shrink-0', tintText('blue'))}
      >
