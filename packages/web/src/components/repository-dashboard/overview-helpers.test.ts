...
describe('buildWorktreeRows', () => {
  ...
  it('resolves a ticket from the server refs even with no live session, enriching the orphaned row', () => {
    const refs = [{ worktreePath: '/wt/f1', id: 't9', displayId: 42, title: 'closed work', status: 'done', type: 'fix', boardId: 'b1' }];
    // no session group, empty client ticket store — resolution must come from the server ref
    const { active, orphaned } = buildWorktreeRows([wt('f1')] as never, refs as never, {}, undefined, [], []);
    expect(active).toHaveLength(0);
    expect(orphaned).toHaveLength(1);
    expect(orphaned[0]!.ticket).toMatchObject({ displayId: 42, status: 'done', title: 'closed work' });
  });

  it('prefers the server ref over the session-group ticket', () => {
    const group = { worktrees: [{ path: '/wt/f1', ticketId: 't1', sessions: [] }] } as never;
    const refs = [{ worktreePath: '/wt/f1', id: 'server-id', displayId: 7, title: 'from manifest', status: 'doing', type: null, boardId: 'b1' }];
    const { active } = buildWorktreeRows([wt('f1')] as never, refs as never, {}, group, [ticket('t1', 'doing')], []);
