...
export function TicketDeliverables({ ticketId }: { ticketId: string }) {
  ...
  const handleToggleRead = useCallback(
    (d: TicketDeliverable, isSeen: boolean) => {
                  ...
                  return (
                    <div className="flex-shrink-0 py-2.5 pl-2">
                      <DeliverableTypePicker
                        deliverable={d}
                        onChanged={(u) =>
                          setDeliverables((prev) => prev.map((x) => (x.id === u.id ? u : x)))
                        }
                      >
                        <span
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      {d.status === 'draft' && (
                        <span
                          className={`flex-shrink-0 rounded-full px-1.5 py-px text-[10px] font-medium ${tint('yellow')}`}
                        >
                          draft
                        </span>
                      )}
                  ...
                  {contentIsUrl ? (
                    <svg
                      className="h-3.5 w-3.5 flex-shrink-0 text-[var(--theme-text-faint)]"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
                    ...
                    <svg
                      className={`h-3.5 w-3.5 flex-shrink-0 ${tintText('blue')}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
