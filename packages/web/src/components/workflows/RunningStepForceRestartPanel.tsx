...
export function RunningStepForceRestartPanel({ startedAt, onForceRestart }: Props) {
  ...
  return (
    <div className={`space-y-3 rounded-md border ${tintClasses('blue').borderColor} ${tintClasses('blue').bg} p-3`}>
      <div>
        ...
        <div className="mt-2 text-xs text-[var(--theme-text-secondary)]">
          Running for {formatDuration(startedAt)}. If the underlying process was
          killed (server hot-reload, crash, or the step is simply stuck), force
          restart to cancel this attempt and start a fresh one.
        </div>
      </div>
