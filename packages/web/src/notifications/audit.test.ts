...
import { auditEntryToWsMessage, pulseEventPrefixes, reconstructNotifications } from './audit';
...
describe('auditEntryToWsMessage', () => {
  ...
  it('maps mention.waiting_for_info and renames mentionId → id', () => {
    const msg = auditEntryToWsMessage(
      entry('mention.waiting_for_info', {
        mentionId: 'm1',
        ticketId: 't1',
        targetAgent: 'the-builder',
      }),
    );
    expect(msg).toEqual({
  ...
  it('passes workflow payloads through unchanged (fields already match 1:1)', () => {
    ...
    expect(auditEntryToWsMessage(entry('workflow.needs_review', payload))?.type).toBe(
      'workflow:needs_review',
    );
    expect(auditEntryToWsMessage(entry('workflow.run_completed', payload))?.type).toBe(
  ...
  it('stamps each notification with the event occurredAt (not "now")', () => {
    const out = reconstructNotifications(
      [
        entry(
          'deliverable.created',
          { deliverableId: 'd1', ticketId: 't1', status: 'final', title: 'R' },
    ...
    const out = reconstructNotifications(
      [
        ...
        entry('deliverable.updated', {
          ...
          newStatus: 'draft',
          title: 'X',
        }), // draft update → renderer opts out
      ],
      reg,
      ctx,
    );
  ...
  it('deduplicates by notification id, keeping the newest occurrence', () => {
    const out = reconstructNotifications(
      [
        entry(
          'deliverable.created',
          { deliverableId: 'd1', ticketId: 't1', status: 'final', title: 'old' },
          '2026-06-01T00:00:00.000Z',
        ),
        entry(
          'deliverable.updated',
          { deliverableId: 'd1', ticketId: 't1', newStatus: 'final', title: 'new' },
