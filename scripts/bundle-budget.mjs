...
const manifest = readJson(
  MANIFEST_PATH,
  'Run `bun run build` first (needs build.manifest in vite.config.ts).',
);
const chunkModules = readJson(
  CHUNK_MODULES_PATH,
  'Run `bun run build` first (emitted by bundleReportPlugin).',
);
...
const candidates = shells.length
  ? shells
  : [{ name: 'entry', js: bootstrap.js, css: bootstrap.css }];
...
const delta = (now, before) =>
  before
    ? ` (${now - before >= 0 ? '+' : ''}${(now - before).toFixed(2)} vs ${before} baseline)`
    : '';
...
lines.push('');
lines.push(
  'Transitive static closure from the entry plus the platform shell. Dynamic imports excluded.',
);
lines.push('');
